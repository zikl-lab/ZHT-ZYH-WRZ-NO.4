...
<div class="panel admin-panel">
    ...
    <table class="table table-hover text-center">
        ...
        <c:forEach items="${pageInfos.list}" var="doc" varStatus="vs">
            <c:choose>
            ...
            <td colspan="8">
                <div class="pagelist">
                    ...
                    <c:if test="${not pageInfos.isFirstPage}">
                        <a href="/doc/docinflist?pageNum=${pageInfo.prePage}">上一页</a>
                    </c:if>
                    <c:forEach items="${pageInfos.navigatepageNums}" var="i">
                        <c:choose>
                            <c:when test="${i == pageInfos.pageNum}">
                                <a class="active-page" href="/doc/docinflist?pageNum=${i}">${i}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="/doc/docinflist?pageNum=${i}">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    ...
                    <c:if test="${not pageInfos.isLastPage}">
                        <a href="/doc/docinflist?pageNum=${pageInfos.nextPage}">下一页</a>
                    </c:if>
                    <a href="/doc/docinflist?pageNum=${pageInfos.pages}">尾页</a>
                </div>
            </td>
        </tr>
    </table>
